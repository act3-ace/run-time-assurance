    ...
    def _setup_docking_constraints(
        ...
        return OrderedDict(
            [
                (
                    "rel_vel",
                    ConstraintDocking2dRelativeVelocity(
                        v0=v0, v1=v1, v0_distance=v0_distance, bias=-1e-4
                    ),
                ),
                (
                    "x_vel",
                    ConstraintMagnitudeStateLimit(limit_val=x_vel_limit, state_index=2),
                ),
                (
                    "y_vel",
                    ConstraintMagnitudeStateLimit(limit_val=y_vel_limit, state_index=3),
                ),
            ]
        )
...
class Docking2dExplicitSwitchingRTA(ExplicitSimplexModule, Docking2dRTAMixin):
    ...
    def _setup_constraints(self) -> OrderedDict:
        return self._setup_docking_constraints(
            self.v0, self.v1, self.v0_distance, self.x_vel_limit, self.y_vel_limit
        )
...
class Docking2dStopLQRBackupController(RTABackupController):
    ...
    def __init__(self, m: float = defaults.M_DEFAULT, n: float = defaults.N_DEFAULT):
        # LQR Gain Matrices
        self.Q = jnp.multiply(0.050, jnp.eye(4))
...
class ConstraintDocking2dRelativeVelocity(ConstraintModule):
    ...
    def _compute(self, state: jnp.ndarray, params: dict) -> float:
        return (
            params["v0"]
            + params["v1"]
            * (norm_with_delta(state[0:2], self.delta) - params["v0_distance"])
            - norm_with_delta(state[2:4], self.delta)
        )
