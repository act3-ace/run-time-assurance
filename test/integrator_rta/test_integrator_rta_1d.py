...
from run_time_assurance.zoo.integrators.integrator_1d import (
    Integrator1dDockingExplicitSwitchingRTA,
    Integrator1dDockingImplicitSwitchingRTA,
    Integrator1dDockingExplicitOptimizationRTA,
    Integrator1dDockingImplicitOptimizationRTA,
)
...
class Env(DataTrackingSampleTestingModule):
    def __init__(self, rta):
        ...
        super().__init__(
            rta=rta, simulation_time=4, step_size=0.02, control_dim=1, state_dim=2
        )
    ...
    def _check_done_conditions(self, state, time):
        docking_done = (
            abs(state[0]) < self.docking_region and state[1] < self.docking_max_vel
        )
    ...
    def plotter(self, array, control, intervening):
        ...
        ax1.fill_between(
            lim_x1, 0, lim_x2, color=(244 / 255, 249 / 255, 241 / 255)
        )  # green
        ax1.fill_between(
            lim_x1, -1, 0, color=(244 / 255, 249 / 255, 241 / 255)
        )  # green
        ax1.fill_between(
            lim_x1, lim_x2, 10, color=(255 / 255, 239 / 255, 239 / 255)
        )  # red
        ...
        ax2.plot([0, xlim], [-1, -1], "k--", linewidth=2)
        ax2.fill_between(
            [0, xlim], -1, 1, color=(244 / 255, 249 / 255, 241 / 255)
        )  # green
        ax2.fill_between(
            [0, xlim], -2, -1, color=(255 / 255, 239 / 255, 239 / 255)
        )  # red
        ax2.fill_between(
            [0, xlim], 1, 2, color=(255 / 255, 239 / 255, 239 / 255)
        )  # red
        ax2.plot(
            np.array(range(len(control[:, 0]))) * self.step_size,
            control[:, 0],
            "b",
            linewidth=2,
        )
...
if __name__ == "__main__":
    ...
    output_names = [
        "rta_test_integrator_1d_explicit_switching",
        "rta_test_integrator_1d_implicit_switching",
        "rta_test_integrator_1d_explicit_optimization",
        "rta_test_integrator_1d_implicit_optimization",
    ]
